...
import { upsertDepartment, toggleDepartment, deleteDepartment } from "./actions";

interface Dept { id: string; name: string; userId: string; isActive: boolean; foremanName: string; employeeCount: number }
interface Foreman { id: string; fullName: string }

export function DepartmentsClient({ departments, foremen }: { departments: Dept[]; foremen: Foreman[] }) {
  ...
  function openEdit(row: Dept) { setAdding(false); setName(row.name); setUserId(row.userId); setError(""); setEditing(row); }
  ...
  function submit() {
    ...
    startTransition(async () => {
      try { await upsertDepartment({ id: editing?.id, name, userId: userId || undefined }); closeForm(); }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h1 className="text-base font-semibold tracking-tight text-foreground">Подразделения</h1>
      ...
      {(adding || editing) && (
        <div className="rounded-xl border border-border bg-card p-5 shadow-sm flex flex-col gap-3 max-w-sm">
          <p className="text-sm font-medium text-foreground">{editing ? "Изменить подразделение" : "Новое подразделение"}</p>
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="dept-name">Название</Label>
            <Input id="dept-name" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && submit()} autoFocus />
          </div>
          <div className="flex flex-col gap-1.5">
              ...
              <tr key={row.id} className="border-b border-border/60 last:border-0 hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-2.5">
                  <div className="flex items-center gap-2 justify-end">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => startTransition(() => toggleDepartment(row.id, !row.isActive))} className="h-7 px-2 text-xs text-zinc-400" disabled={pending}>{row.isActive ? "В архив" : "Восстановить"}</Button>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => {
                        const msg = row.employeeCount > 0
                          ? `Удалить подразделение «${row.name}»?\n\nБудет удалено ${row.employeeCount} сотрудник(ов) вместе с их строками табелей.`
                          : `Удалить подразделение «${row.name}»?`;
                        if (confirm(msg)) startTransition(() => deleteDepartment(row.id));
                      }}
